    ...
    if row_dict['tx_type'] in ["TRADE", "_BOND"]:
        if len(row_dict['fee_currency']) > 0:
            ...
            data_row.t_record = TransactionOutRecord(TransactionOutRecord.TYPE_TRADE,
                                                     ...
                                                     sell_asset=row_dict['sent_currency'],
                                                     wallet=wallet_name(row_dict['wallet_address']))
    elif row_dict['tx_type'] in ["_LP_DEPOSIT", "_LP_WITHDRAW", "LP_DEPOSIT", "LP_WITHDRAW"]:
        if len(row_dict['fee_currency']) > 0:
            data_row.t_record = TransactionOutRecord(TransactionOutRecord.TYPE_TRADE,
                                                 data_row.timestamp,
                                                 buy_quantity=row_dict['received_amount'],
...
DataParser(DataParser.TYPE_WALLET,
           "StakeTax",
           ['timestamp','tx_type','received_amount','received_currency','sent_amount','sent_currency','fee','fee_currency','comment','txid','url','exchange','wallet_address'],
           worksheet_name="StakeTax",
           row_handler=parse_staketax)
